...
instance Monad RWSP where
  ...
  -- m >>= f = undefined
  m >>= f = RWSP $ \r s -> let (a, w, newState) = runRWSP m r s
                               (a1, w1, newState1) = runRWSP (f a) r newState 
                           in (a1, w `mappend` w1, newState1)
...
instance Monad RWSE where
  ...
  m >>= f = RWSE $ \r s -> do
                           (a, w, s1) <- runRWSE m r s
                           (a1, w1, s2) <- runRWSE (f a) r s1
                           return (a1, w `mappend` w1, s2)
                           
  -- n >>= f = RWSE $ \r s -> case runRWSE n r s of Left err -> Left err
  -- m >>= f = RWSE $ \r s -> case runRWSE m r s of 
  --                               Left err -> Left err
  --                               Right (a, w, s) -> case Right case runRWSE (f a) r s' of
  --                                                       Left err -> Left err
  --                                                       Right (a', w', s'') -> return (a', w `mappend` w', s'')
